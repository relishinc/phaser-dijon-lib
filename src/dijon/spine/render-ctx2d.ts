...
export class RenderCtx2D {
  ...
  public loadData(spine_data: spine.Data, atlas_data: atlas.Data, images: {[key: string]: HTMLImageElement}): void {
    this.images = images; 
    spine_data.iterateSkins((skin_key: string, skin: spine.Skin)=> {
      const skin_info: SkinInfo = this.skin_info_map[skin_key] = new SkinInfo();
  ...
  public updatePose(spine_pose: spine.Pose, atlas_data: atlas.Data): void {
    spine_pose.iterateAttachments((slot_key: string, slot: spine.Slot, skin_slot: spine.SkinSlot, attachment_key: string, attachment: spine.Attachment)=> {
      if (!attachment) { return; }
      const skin_info: SkinInfo = this.skin_info_map[spine_pose.skin_key];
      const default_skin_info: SkinInfo = this.skin_info_map['default'];
  ...
  public drawPose(spine_pose: spine.Pose, atlas_data: atlas.Data): void {
    ...
    spine_pose.iterateAttachments((slot_key: string, slot: spine.Slot, skin_slot: spine.SkinSlot, attachment_key: string, attachment: spine.Attachment)=> {
      ...
      this.ctx.save();
      // slot.color.rgb
      //this.ctx.globalAlpha *= slot.color.a;
      switch (slot.blend) {
        default:
        case 'normal': this.ctx.globalCompositeOperation = 'source-over'; break;
        case 'additive': this.ctx.globalCompositeOperation = 'lighter'; break;
      ...
      switch (attachment.type) {
        case 'region': {
          ...
          ctxApplySpace(this.ctx, bone.world_space);
          ctxApplySpace(this.ctx, region_attachment.local_space);
          ctxApplyAtlasSitePosition(this.ctx, site);
          this.ctx.scale(region_attachment.width / 2, region_attachment.height / 2);
          ctxDrawImageMesh(this.ctx, this.region_vertex_triangle, this.region_vertex_position, this.region_vertex_texcoord, image, site, page);
          break;
        }
        case 'mesh': {
          const bone: spine.Bone = spine_pose.bones[slot.bone_key];
          ...
          ctxApplySpace(this.ctx, bone.world_space);
          ctxApplyAtlasSitePosition(this.ctx, site);
          ctxDrawImageMesh(this.ctx, mesh_attachment_info.vertex_triangle, mesh_attachment_info.vertex_position, mesh_attachment_info.vertex_texcoord, image, site, page);
          break;
        }
        case 'skinnedmesh': {
          const skinned_mesh_attachment: spine.SkinnedMeshAttachment = <spine.SkinnedMeshAttachment>attachment;
          const skinned_mesh_attachment_info: SkinnedMeshAttachmentInfo = <SkinnedMeshAttachmentInfo>slot_info.attachment_info_map[attachment_key];
          ctxApplyAtlasSitePosition(this.ctx, site);
          ctxDrawImageMesh(this.ctx, skinned_mesh_attachment_info.vertex_triangle, skinned_mesh_attachment_info.vertex_blend_position, skinned_mesh_attachment_info.vertex_texcoord, image, site, page);
          break;
        }
        default:
          break;
