...
export class Application implements INotifier {
  ...
  public static textResolution: number = 1;
  // static constants
    ...
    if (
      ...
      this.game.input.touch.addTouchLockCallback(() => {
        ...
        if (this.game.sound.usingWebAudio) {
          ...
          this.game.sound[
            "unlockSource"
          ] = this.game.sound.context.createBufferSource();
          this.game.sound["unlockSource"].buffer = buffer;
          this.game.sound["unlockSource"].connect(
            this.game.sound.context.destination
          );
  ...
  public trackEvent(eventModel: AnalyticsEventModel): void {
    this.game.analytics.trackEvent(
      eventModel.action,
      eventModel.label,
      eventModel.value === null ? null : eventModel.value.toString()
    );
  }

  public trackEventAndChangeCategory(
    newCategory: string,
  ...
  public registerModel(model: Model): Model {
    if (this._models[model.name]) {
      throw new Error(
        'Application:: a model with the name "' +
          model.name +
          '" already exists.'
      );
    }
  ...
  public registerMediator(mediator: Mediator): void {
    if (this._mediators[mediator.name]) {
      throw new Error(
        'Application:: a mediator with the name "' +
          mediator.name +
          '" already exists.'
      );
    }
